...
public class LeverSwapPanel4Panel5 : MonoBehaviour {
	...
	void Start () {
		...
		GameObject p1_1_points = PhotonNetwork.Instantiate("p1_4_points", q, Quaternion.identity, 0);
...
//Find what I wanna destroy
		GameObject destroyThis = GameObject.Find("p1_3_points(Clone)");
	...
	void Player2View(){
		...
		GameObject p2_1_points = PhotonNetwork.Instantiate("p2_4_points", Vector3.zero, Quaternion.identity, 0);
		GameObject player2prefab = PhotonNetwork.Instantiate("p2LeverReadyS", Vector3.zero, Quaternion.identity, 0);
		GameObject destroyThis = GameObject.Find("p2LeverReadyQ(Clone)");
		PhotonNetwork.Destroy(destroyThis.GetComponent<PhotonView>());

		GameObject destroyThat = GameObject.Find("p2_3_points(Clone)");
